...
def test_hdf5_path_prefix_default(clitest, testprocess):
    ...
    clitest.expect_filename_pattern(r"^goeffel-timeseries__[0-9]+-[0-9]+\.hdf5$")


def test_hdf5_path_with_label(clitest, testprocess):
    clitest.run(f"goeffel --pid {testprocess.pid} -i 0.3 -t 1 " "--label custom-label")
...
def test_hdf5_path_prefix_custom(clitest, testprocess):
    ...
    clitest.expect_filename_pattern(r"^custom_prefix__[0-9]+-[0-9]+\.hdf5$")
...
def test_hdf5_path_prefix_custom_and_label(clitest, testprocess):
    ...
    clitest.expect_filename_pattern(r"^custom_prefix_custom-label_[0-9]+-[0-9]+\.hdf5$")
...
def test_hdf5_path(clitest, testprocess):
    ...
    clitest.expect_filename_pattern(r"^out\.hdf5$")
...
def test_analysis_inspect(clitest, testprocess):
    ...
    clitest.run(f"goeffel-analysis inspect out.hdf5")
    clitest.assert_in_stdout(
        ["Created with: Goeffel", "Table properties:", "Column names:"]
    )
...
def test_analysis_plot(clitest, testprocess):
    ...
    clitest.run(f"goeffel-analysis plot out.hdf5")
...
def test_analysis_plot_additional_metric(clitest, testprocess):
    ...
    clitest.run(f"goeffel-analysis plot out.hdf5 --metric system_loadavg1")
    clitest.assert_in_stderr(["Writing figure as PNG to"])
